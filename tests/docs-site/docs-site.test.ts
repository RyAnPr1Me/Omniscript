...
describe("StaticDocGenerator", () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(
      path.join(testSourceDir, "README.md"),
      "# Test Documentation\n\nThis is a test.",
    );
    fs.writeFileSync(
      path.join(testSourceDir, "guide.md"),
      "## Getting Started\n\nQuick start guide.",
    );
  });
  ...
  test("generateSite creates required files", async () => {
    ...
    expect(fs.existsSync(path.join(testOutputDir, "css", "styles.css"))).toBe(
      true,
    );
  ...
  test("generateSite converts markdown to HTML", async () => {
    ...
    const readmeHtml = fs.readFileSync(
      path.join(testOutputDir, "README.html"),
      "utf-8",
    );
  ...
  test("CSS file contains required styles", async () => {
    ...
    const cssContent = fs.readFileSync(
      path.join(testOutputDir, "css", "styles.css"),
      "utf-8",
    );
  ...
  test("JavaScript file contains required functionality", async () => {
    ...
    const jsContent = fs.readFileSync(
      path.join(testOutputDir, "js", "main.js"),
      "utf-8",
    );
  ...
  test("versions.json contains correct version info", async () => {
    ...
    const versionsContent = fs.readFileSync(
      path.join(testOutputDir, "versions.json"),
      "utf-8",
    );
