...
describe('Event Sourcing System', () => {
  ...
  test('save and load aggregate from repository', async () => {
    ...
    // Load aggregate
    const loadedUser = await repository.getById('user-456', (id: string) => new UserAggregate(id));
  ...
  test('event store operations', async () => {
    const events: DomainEvent[] = [
      ...
      },
      {
        ...
        aggregateId: 'user-789',
        aggregateType: 'User',
        payload: { oldEmail: 'bob@example.com', newEmail: 'bob.wilson@example.com' },
        metadata: {},
        timestamp: Date.now(),
        version: 2
      }
    ...
    // Get events by type
    const registrationEvents = await eventStore.getEventsByType('UserRegistered');
  ...
  test('projections', async () => {
    ...
    const projectionData: Record<string, UserProjection> = {};
  ...
  test('snapshots', async () => {
    ...
    const loadedSnapshot = await snapshotStore.get<{ name: string; email: string; isActive: boolean }>('user-snapshot');
