...
describe('Genetic Algorithm Module', () => {
  ...
  describe('Crossover Strategies', () => {
    test('should work with uniform crossover', () => {
      const fitnessFunction = (params: number[]) => params.reduce((sum, x) => sum + x, 0);
  ...
  describe('Constraints and Bounds', () => {
    test('should respect parameter constraints', () => {
      ...
      expect(result.bestParams[0] + result.bestParams[1]).toBeLessThanOrEqual(2.5); // Even more tolerance for constraint handling
  ...
  describe('Advanced Features', () => {
    ...
    test('should support adaptive mutation', () => {
      ...
      const lastMutationRate = result.convergenceHistory[result.convergenceHistory.length - 1].mutationRate;
