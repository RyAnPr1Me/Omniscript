...
describe('Standard Library - Path', () => {
  ...
  test('join combines path segments', () => {
    ...
    expect(Path.join('/a', 'b', 'c')).toBe(Path.sep + 'a' + Path.sep + 'b' + Path.sep + 'c');
  ...
  test('format reconstructs path from components', () => {
    ...
    expect(formatted).toBe(Path.sep + 'home' + Path.sep + 'user' + Path.sep + 'file.txt');
  });
  ...
  test('relative calculates relative path', () => {
    ...
    expect(relative).toBe('..' + Path.sep + '..' + Path.sep + 'd' + Path.sep + 'e');
  });
  ...
  test('changeExtension modifies file extension', () => {
    expect(Path.changeExtension('/path/file.txt', '.md')).toBe(Path.sep + 'path' + Path.sep + 'file.md');
  ...
  test('addSuffix adds suffix before extension', () => {
    expect(Path.addSuffix('/path/file.txt', '-backup')).toBe(Path.sep + 'path' + Path.sep + 'file-backup.txt');
    expect(Path.addSuffix('file', '-backup')).toBe('file-backup');
  });

  test('addPrefix adds prefix to filename', () => {
    expect(Path.addPrefix('/path/file.txt', 'old-')).toBe(Path.sep + 'path' + Path.sep + 'old-file.txt');
  ...
  test('handles multiple consecutive separators', () => {
    expect(Path.normalize('a//b///c')).toBe('a' + Path.sep + 'b' + Path.sep + 'c');
    expect(Path.join('a', '', '', 'b')).toBe('a' + Path.sep + 'b');
  });
