import { OmniCodec, MediaUtils, omniCodec, MediaHeader } from '../../src/stdlib/media';
...
describe('OmniCodec Media Encoding', () => {
  ...
  describe('Audio Encoding', () => {
    ...
    test('should handle different quality settings', async () => {
      ...
      // Test high quality
      const highQuality = await omniCodec.encode(originalData, 'audio', metadata, { quality: 95 });
      
      // Test low quality  
      const lowQuality = await omniCodec.encode(originalData, 'audio', metadata, { quality: 30 });
  ...
  describe('SIMD Performance', () => {
    test('should handle SIMD vs non-SIMD encoding', async () => {
      ...
      const simdEncoded = await omniCodec.encode(originalData, 'audio', metadata, {
        enableSIMD: true,
        quality: 80
      });
      ...
      const normalEncoded = await omniCodec.encode(originalData, 'audio', metadata, {
        enableSIMD: false,
        quality: 80
      });
  ...
  describe('Error Handling', () => {
    test('should handle invalid data gracefully', async () => {
      ...
      await expect(omniCodec.decode(invalidData)).rejects.toThrow('Invalid OmniCodec file format');
    });
    ...
    test('should validate quality parameters', async () => {
      ...
      const minQuality = await omniCodec.encode(testData, 'audio', metadata, { quality: 1 });
      const maxQuality = await omniCodec.encode(testData, 'audio', metadata, { quality: 100 });
  ...
  describe('Encryption Support', () => {
    test('should handle encryption flag', async () => {
      ...
      const encrypted = await omniCodec.encode(originalData, 'video', metadata, {
  ...
  describe('Format Compatibility', () => {
    ...
    test('should handle mixed audio and video encoding', async () => {
      ...
      const encodedAudio = await omniCodec.encode(audioData, 'audio', audioMetadata);
      const encodedVideo = await omniCodec.encode(videoData, 'video', videoMetadata);
