...
describe('DateTime Library', () => {
  ...
  describe('DateTimeUtils', () => {
    ...
    it('should handle timeout function', async () => {
      ...
      await expect(DateTimeUtils.timeout(fastPromise, 100)).resolves.toBe('success');
      await expect(DateTimeUtils.timeout(slowPromise, 100)).rejects.toThrow('Operation timed out');
    });
  });
});