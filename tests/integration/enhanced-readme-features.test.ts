...
describe("Enhanced README Features", () => {
  ...
  test("pattern matching with guards works correctly", async () => {
    const src =
      'let value = 5; match value { 0 => "zero", n if n > 0 => "positive", n if n < 0 => "negative", _ => "unknown" }';
  ...
  test("pattern matching with guards for negative numbers", async () => {
    const src =
      'let value = -3; match value { 0 => "zero", n if n > 0 => "positive", n if n < 0 => "negative", _ => "unknown" }';
    const result = await omni.execute(src);
  ...
  test("pattern matching with guards for zero", async () => {
    const src =
      'let value = 0; match value { 0 => "zero", n if n > 0 => "positive", n if n < 0 => "negative", _ => "unknown" }';
    const result = await omni.execute(src);
  ...
  test("method decorators are parsed and stored", async () => {
    const src =
      'class UserList { @state getUsers() => "users", @computed getActiveUsers() => "active" }';
    const result = await omni.execute(src);
  ...
  test("string literals work in pattern matching", async () => {
    const src =
      'match "hello" { "world" => "no", "hello" => "yes", _ => "unknown" }';
    const result = await omni.execute(src);
  ...
  test("operator overloading with improved math", async () => {
    const src =
      "class Num { operator + (other) => add(self.value, other.value) } let a = new Num(2); let b = new Num(3); a + b";
    const result = await omni.execute(src);
