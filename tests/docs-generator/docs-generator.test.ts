  ...
  describe("TypeScriptDocGenerator", () => {
    ...
    test("should extract class information", () => {
      ...
      const testModule = modules.find(
        (m) =>
          m.name.includes("test-file") ||
          m.classes.some((c) => c.name === "TestClass"),
      );

      if (testModule) {
        expect(testModule.classes.length).toBeGreaterThan(0);

        const testClass = testModule.classes.find(
          (c) => c.name === "TestClass",
        );
        if (testClass) {
          ...
          expect(testClass.documentation.description).toContain(
            "Test class for documentation generation",
          );
        }
      }
    ...
    test("should extract interface information", () => {
      ...
      const testModule = modules.find((m) =>
        m.interfaces.some((i) => i.name === "TestInterface"),
      );

      if (testModule) {
        const testInterface = testModule.interfaces.find(
    ...
    test("should extract function information", () => {
      ...
      const testModule = modules.find((m) =>
        m.functions.some((f) => f.name === "testFunction"),
      );

      if (testModule) {
        const testFunction = testModule.functions.find(
    ...
    test("should extract JSDoc comments", () => {
      ...
      if (testModule) {
        ...
        if (testClass) {
          ...
          const testMethod = testClass.methods.find(
            (m) => m.name === "testMethod",
          );
          if (testMethod) {
  ...
  describe("MarkdownDocGenerator", () => {
    ...
    test("should generate proper markdown formatting", () => {
      ...
      expect(markdown).toContain(
        "**Returns**: `Promise<void>` - Promise that resolves",
      );
    });
  });
});
