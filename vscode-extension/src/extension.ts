...
export function activate(context: vscode.ExtensionContext) {
  ...
  const compileProjectCommand = vscode.commands.registerCommand(
    "omniscript.compileProject",
    async () => {
  ...
  const checkTypesCommand = vscode.commands.registerCommand(
    "omniscript.checkTypes",
    async (uri?: vscode.Uri) => {
  ...
  const formatProvider =
    vscode.languages.registerDocumentFormattingEditProvider("omniscript", {
  ...
  const diagnosticCollection =
    vscode.languages.createDiagnosticCollection("omniscript");
...
async function compileFile(uri?: vscode.Uri): Promise<void> {
  ...
  if (!compilerPath) {
    vscode.window.showErrorMessage(
      "Omniscript CLI not found. Please install it first.",
    );
    return;
  }

  const args = ["compile", filePath, "--output", outputDir, "--target", target];
    ...
    async (progress) => {
      return new Promise<void>((resolve, reject) => {
        ...
        process.on("error", (error) => {
          vscode.window.showErrorMessage(
            `Error running type checker: ${error.message}`,
          );
          reject(error);
        });
      });
...
async function formatDocument(): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    process.on("error", (error) => {
      vscode.window.showErrorMessage(
        `Error running formatter: ${error.message}`,
      );
      reject(error);
    });
  });
...
async function createNewProject(): Promise<void> {
  ...
  vscode.window.showInformationMessage(
    `Creating new Omniscript project: ${projectName}`,
  );
    ...
    [
      {
        ...
        description: "npm install -g omniscript",
      },
      {
        label: "Install from source",
        description: "Build and install from Omniscript repository",
      },
  ...
  switch (choice.label) {
    ...
    case "Download binary":
      vscode.env.openExternal(
        vscode.Uri.parse("https://github.com/RyAnPr1Me/Omniscript/releases"),
      );
      break;
  }
