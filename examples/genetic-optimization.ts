...
const simpleResult = optimize((params: number[]) => -((params[0] - 5) ** 2), {
  parameterCount: 1,
  populationSize: 30,
  maxGenerations: 50,
...
console.log(
  `Optimal parameters: [${constrainedResult.bestParams.map((p) => p.toFixed(4)).join(", ")}]`,
);
console.log(`Best fitness: ${constrainedResult.bestFitness}`);
console.log(
  `Sum constraint satisfied: ${constrainedResult.bestParams[0] + constrainedResult.bestParams[1] <= 1.1}`,
);
console.log();
...
console.log(
  `  Learning rate: ${hyperparameterResult.bestParams[0].toFixed(6)}`,
);
...
console.log(
  `Pareto optimal solution: ${multiObjectiveResult.bestParams[0].toFixed(4)}`,
);
console.log(
  `Objective values: [${(multiObjectiveResult.bestFitness as number[]).map((f) => f.toFixed(4)).join(", ")}]`,
);
console.log();
...
console.log(
  `Global optimum found: [${rastriginResult.bestParams.map((p) => p.toFixed(4)).join(", ")}]`,
);
...
const convergenceSteps = Math.min(
  10,
  rastriginResult.convergenceHistory.length,
);
const stepSize = Math.floor(
  rastriginResult.convergenceHistory.length / convergenceSteps,
);
...
for (let i = 0; i < convergenceSteps; i++) {
  ...
  console.log(
    `  Gen ${stats.generation}: Best=${(stats.bestFitness as number).toFixed(4)}, Diversity=${stats.diversity.toFixed(4)}, MutRate=${stats.mutationRate.toFixed(4)}`,
  );
}
