...
export default class OmniscriptLexer extends Lexer {
  ...
  nextToken(): any {
    try {
      ...
      const isWS = (code: number) => code === 32 || code === 9 || code === 10 || code === 13;
      const isAlpha = (code: number) => (code >= 65 && code <= 90) || (code >= 97 && code <= 122) || code === 95; // A-Z a-z _
      const isNum = (code: number) => code >= 48 && code <= 57;

      const consume = () => { if (this._input) this._input.consume(); };
      ...
      while (this._input) {
        ...
        if (C() === '/' && C2() === '*') {
          consume(); consume();
          while (this._input && !(C() === '*' && C2() === '/')) { if (ch() === -1) break; consume(); }
          if (C() === '*' && C2() === '/') { consume(); consume(); }
          continue;
        }
        break;
      }
      ...
      if (C() === '"') {
        ...
        while (this._input && ch() !== -1 && C() !== '"') {
          if (C() === '\\') { text += '\\'; consume(); if (ch() !== -1) { text += C(); consume(); continue; } }
          text += C(); consume();
        }
        if (C() === '"') { text += '"'; consume(); }
      ...
      if (isNum(ch())) {
        let txt = '';
        while (isNum(ch())) { txt += C(); consume(); }
        if (C() === '.') { txt += '.'; consume(); while (isNum(ch())) { txt += C(); consume(); } }
      ...
      if (isAlpha(ch())) {
        ...
        switch (kw) {
          ...
          case 'while': return this.makeToken(OmniscriptParser.WHILE, kw);
          case 'for': return this.makeToken(OmniscriptParser.FOR, kw);
          case 'try': return this.makeToken(OmniscriptParser.TRY, kw);
          case 'catch': return this.makeToken(OmniscriptParser.CATCH, kw);
          case 'finally': return this.makeToken(OmniscriptParser.FINALLY, kw);
          case 'throw': return this.makeToken(OmniscriptParser.THROW, kw);
          case 'async': return this.makeToken(OmniscriptParser.ASYNC, kw);
          ...
          default: return this.makeToken(OmniscriptParser.IDENTIFIER, id);
        }
      }
      ...
      if (C() === '>' && C2() === '=') { consume(); consume(); return this.makeToken(OmniscriptParser.GTE, '>='); }
      if (C() === '&' && C2() === '&') { consume(); consume(); return this.makeToken(OmniscriptParser.AND, '&&'); }
      if (C() === '|' && C2() === '|') { consume(); consume(); return this.makeToken(OmniscriptParser.OR, '||'); }
  ...
  ';': OmniscriptParser.SEMI,
      ...
      const t = single[C()];
      if (t !== undefined) { const txt = C(); consume(); return this.makeToken(t, txt); }
