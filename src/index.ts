...
export class Omniscript {
  ...
  async execute(source: string): Promise<any> {
    ...
    const hasFunctionalSyntax =
      /fn\s*\([^)]*\)\s*=>/g.test(source) ||
      ...
      /\b(curry|memoize|just|nothing|left|right|head|tail|cons|flip)\s*\(/.test(
        source,
      ) || // Functional builtins (removed add/inc as they're too common)
