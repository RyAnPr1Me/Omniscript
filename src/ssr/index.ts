...
export class SSRRenderer {
  ...
  private initializeRenderer(): void {
    debug.debug(
      "SSR",
      `Initializing SSR renderer with entry: ${this.config.entry}`,
    );
  ...
  async render(context: RenderContext): Promise<RenderResult> {
    try {
      ...
      const mockApp = {
        render: () => ({ toString: () => "<div>SSR App</div>" }),
      };
      const app = mockApp;
  ...
  private matchRoute(
    url: string,
  ): { component?: string; redirect?: string } | null {
  ...
  private applyTemplate(
    html: string,
    assets: { css?: string; js?: string; meta?: any },
  ): string {
    ...
    template = template.replace(
      "{{TITLE}}",
      assets.meta?.title || "Omniscript SSR",
    );
    template = template.replace(
      "{{META}}",
      this.generateMetaTags(assets.meta || {}),
    );
    template = template.replace(
      "{{CSS}}",
    ...
    template = template.replace(
      "{{JS}}",
      assets.js ? `<script>${assets.js}</script>` : "",
    );
...
<body>
  ...
  async generateStaticSite(): Promise<void> {
    ...
    debug.debug(
      "SSR",
      `Generating static site with ${this.config.routes.length} routes`,
    );
  ...
  private async generateStaticPage(
    ...
    try {
      ...
      if (result.redirectTo) {
        debug.debug(
          "SSR",
          `Skipping redirect route: ${route} -> ${result.redirectTo}`,
        );
        return;
      }
      ...
      debug.error(
        "SSR",
        `Failed to generate static page for ${route}: ${error}`,
      );
    }
  }
