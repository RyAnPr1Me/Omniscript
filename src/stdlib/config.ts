...
export class EnvironmentSource implements ConfigSource {
  ...
  private parseValue(value: string): any {
    ...
    if (
      (!isNaN(num) && isFinite(num) && !value.startsWith("0")) ||
      value === "0"
    ) {
      return num;
    }
...
export class FileSource implements ConfigSource {
  ...
  async save(config: Record<string, any>): Promise<void> {
    try {
      const serialized = Serialization.serialize(config, this.format, {
        pretty: true,
        format: this.format as any,
      });
      logger.info("Config", { filename: this.filename, size: serialized.size });
    } catch (error) {
      logger.error("Config - Failed to save file", error as Error, {
  ...
  async save(): Promise<void> {
    ...
    for (const source of writableSources) {
      try {
        ...
        logger.error("Config - Failed to save to source", error as Error, {
          source: source.name,
        });
      }
    }
  ...
  private notifyChange(event: ConfigChangeEvent): void {
    for (const listener of this.listeners) {
      try {
        ...
        logger.error(
          "Config - Configuration change listener failed",
          error as Error,
        );
      }
    }
...
export const ConfigSchemas = {
  server: Validator.object({
    ...
    port: Validator.required(
      Validator.number({ min: 1, max: 65535, integer: true }),
    ),
  ...
  logging: Validator.object({
    level: Validator.required(
      Validator.enum(["trace", "debug", "info", "warn", "error", "fatal"]),
    ),
    outputs: Validator.required(
      Validator.array(Validator.string(), { minLength: 1 }),
    ),
  ...
  app: Validator.object({
    ...
    version: Validator.required(
      Validator.string({ pattern: /^\d+\.\d+\.\d+/ }),
    ),
    environment: Validator.optional(
      Validator.enum(["development", "test", "staging", "production"]),
    ),
  }),
};
