...
export class XmlSerializer extends BaseSerializer {
  ...
  private objectToXml(
    ...
    if (Array.isArray(obj)) {
      const items = obj
        .map((item) => this.objectToXml(item, { pretty, indent: indent + 1 }))
        .join("");
    ...
    if (typeof obj === "object") {
      const entries = Object.entries(obj)
        .map(([key, value]) => {
          ...
          const content = this.objectToXml(value, {
            pretty,
            indent: indent + 1,
          });
      ...
      return indent === 0
        ? `<?xml version="1.0" encoding="UTF-8"?>${newline}<root>${newline}${entries}</root>${newline}`
        : entries;
    }
...
export class BinarySerializer extends BaseSerializer {
  ...
  serialize(data: any, options?: SerializationOptions): SerializationResult {
    try {
      ...
      throw new Error(
        `Binary serialization failed: ${(error as Error).message}`,
      );
    }
  }
  ...
  private readValue(buffer: Uint8Array, context: { offset: number }): any {
    ...
    switch (type) {
      ...
      case 0x11: {
        // int16
        const int16 = new DataView(buffer.buffer, context.offset, 2).getInt16(
          0,
          true,
        );
        context.offset += 2;
  ...
  private readLength(buffer: Uint8Array, context: { offset: number }): number {
    ...
    if (first === 0xff) {
      const length = new DataView(buffer.buffer, context.offset, 2).getUint16(
        0,
        true,
      );
      context.offset += 2;
      ...
      const length = new DataView(buffer.buffer, context.offset, 4).getUint32(
        0,
        true,
      );
      context.offset += 4;
...
export class Serialization {
  ...
  static compare(obj1: any, obj2: any, format: string = "json"): boolean {
    try {
      ...
      if (
        typeof serialized1.data === "string" &&
        typeof serialized2.data === "string"
      ) {
        return serialized1.data === serialized2.data;
      }

      if (
        serialized1.data instanceof Uint8Array &&
        serialized2.data instanceof Uint8Array
      ) {
        return (
          serialized1.data.length === serialized2.data.length &&
          serialized1.data.every(
            (byte, index) => byte === serialized2.data[index],
          )
        );
      }
...
export const Serialize = {
  toJson: (data: any, pretty = false) =>
    Serialization.serialize(data, "json", { pretty, format: "json" }),
  fromJson: <T = any>(json: string) =>
    Serialization.deserialize<T>(json, "json"),

  toXml: (data: any, pretty = false) =>
    Serialization.serialize(data, "xml", { pretty, format: "xml" }),
  fromXml: <T = any>(xml: string) => Serialization.deserialize<T>(xml, "xml"),

  toYaml: (data: any) => Serialization.serialize(data, "yaml"),
  fromYaml: <T = any>(yaml: string) =>
    Serialization.deserialize<T>(yaml, "yaml"),

  toBinary: (data: any) => Serialization.serialize(data, "binary"),
  fromBinary: <T = any>(binary: Uint8Array) =>
    Serialization.deserialize<T>(binary, "binary"),
