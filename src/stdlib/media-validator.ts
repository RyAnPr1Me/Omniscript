...
export class MediaValidator {
  ...
  static validateEncodeInput(
    ...
    if (type !== "audio" && type !== "video") {
      throw new ValidationError(
        `Invalid media type: ${type}. Must be 'audio' or 'video'`,
      );
    }
  ...
  static validateDecodeInput(encodedData: Uint8Array): void {
    ...
    if (encodedData.length < 8) {
      ...
      if (encodedData.length >= 4) {
        ...
        const hasValidMagic = encodedData
          .slice(0, 4)
          .every((byte, i) => byte === magicBytes[i]);
        if (!hasValidMagic) {
          throw new ValidationError("Invalid OmniCodec file format");
        }
      }
      throw new ValidationError(
        "Encoded data is too small to contain valid OmniCodec header",
      );
    }
  }
  ...
  private static validateMetadata(
    ...
    if (
      metadata.version &&
      !this.SUPPORTED_VERSIONS.includes(metadata.version)
    ) {
      throw new ValidationError(
  ...
  private static validateAudioMetadata(metadata: Partial<MediaHeader>): void {
    if (metadata.channels !== undefined) {
      if (
        ...
        metadata.channels < 1 ||
        metadata.channels > this.MAX_CHANNELS
      ) {
        throw new ValidationError(
          `Audio channels must be an integer between 1 and ${this.MAX_CHANNELS}`,
        );
      }
    }
    ...
    if (metadata.sampleRate !== undefined) {
      if (
        ...
        metadata.sampleRate > this.MAX_SAMPLE_RATE
      ) {
        throw new ValidationError(
          `Sample rate must be an integer between 8000 and ${this.MAX_SAMPLE_RATE} Hz`,
        );
      }
    }
    ...
    if (
      ...
      throw new ValidationError(
        "Audio metadata should not contain video properties (width, height, frameRate)",
      );
    }
  }
  ...
  private static validateVideoMetadata(metadata: Partial<MediaHeader>): void {
    if (metadata.width !== undefined) {
      if (
        ...
        throw new ValidationError(
          `Video width must be an integer between 1 and ${this.MAX_DIMENSIONS}`,
        );
      }
    }
    ...
    if (metadata.height !== undefined) {
      if (
        ...
        throw new ValidationError(
          `Video height must be an integer between 1 and ${this.MAX_DIMENSIONS}`,
        );
      }
    }
    ...
    if (metadata.frameRate !== undefined) {
      if (
        ...
        metadata.frameRate > 120
      ) {
        throw new ValidationError(
          "Video frame rate must be a number between 0 and 120 fps",
        );
      }
    }
    ...
    if (metadata.channels !== undefined || metadata.sampleRate !== undefined) {
      throw new ValidationError(
        "Video metadata should not contain audio properties (channels, sampleRate)",
      );
    }
  }
  ...
  private static validateOptions(options: Partial<OmniCodecOptions>): void {
    ...
    if (
      options.enableEncryption !== undefined &&
      typeof options.enableEncryption !== "boolean"
    ) {
      throw new ValidationError("enableEncryption must be a boolean");
    }

    if (
      options.enableSIMD !== undefined &&
      typeof options.enableSIMD !== "boolean"
    ) {
  ...
  static validateHeader(header: MediaHeader): void {
    ...
    if (
      !header.codec ||
      (header.codec !== "OmniCodec" && header.codec !== "OmniCodec-H264")
    ) {
      throw new ValidationError(
        `Invalid codec: ${header.codec}. Expected 'OmniCodec' or 'OmniCodec-H264'`,
      );
    }

    if (typeof header.duration !== "number" || header.duration < 0) {
      throw new ValidationError(
        "Header duration must be a non-negative number",
      );
    }
