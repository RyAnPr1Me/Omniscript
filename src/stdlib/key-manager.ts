...
export class KeyManager {
  ...
  private cleanupOldKeys(): void {
    ...
    if (
      ...
      this.keys.size >= this.options.maxStoredKeys
    ) {
      ...
      for (
        let i = 0;
        i < sortedKeys.length - this.options.maxStoredKeys + 1;
        i++
      ) {
    ...
    if (keysToDelete.length > 0) {
      debug.debug(
        "Media",
        `Cleaned up ${keysToDelete.length} old encryption keys`,
      );
    }
  }
   ...
   */
  getEncryptionInfo(keyData: EncryptionKeyData): {
    keyId: string;
    algorithm: string;
  } {
    return {
