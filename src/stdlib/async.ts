...
export class AsyncUtils {
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      if (executing.length >= maxConcurrency) {
        ...
        const completedIndex = executing.findIndex(
          (p) => (p as any).isFulfilled || (p as any).isRejected,
        );
        if (completedIndex >= 0) {
   ...
   */
  static throttle<T extends (...args: any[]) => void>(fn: T, delay: number): T {
    let lastCall = 0;
      ...
      for (const promise of promises) {
        promise.then(resolve).catch((error) => {
          ...
          if (rejectionCount === promises.length) {
            reject(
              new Error(
                `All promises rejected: ${errors.map((e) => e.message).join(", ")}`,
              ),
            );
          }
        });
   ...
   */
  static async allSettled<T>(promises: Promise<T>[]): Promise<
    Array<{
...
class AsyncPatterns {
   ...
   */
  static async raceN<T>(promises: Promise<T>[], count: number): Promise<T[]> {
    if (count <= 0) return [];
