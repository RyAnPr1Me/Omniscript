...
export type Lambda = { type: 'Lambda'; params: string[]; body: Expression; closure?: Env };
export type Call = { type: 'Call'; callee: Expression; args: Expression[] };
export type Let = { type: 'Let'; name: string; value: Expression; body?: Expression; immutable?: boolean };
export type Const = { type: 'Const'; name: string; value: Expression; body?: Expression };
...
export type CurriedFunction = { type: 'CurriedFunction'; fn: Lambda; appliedArgs: Expression[] };
export type PartialApplication = { type: 'Partial'; fn: Expression; args: (Expression | null)[] };
...
export type Expression = NumberLiteral | StringLiteral | BooleanLiteral | Identifier | Lambda | Call | Let | Const | IfExpr | Pipe | Binary | Unary | Match | AwaitExpr | ReturnExpr | ClassDecl | NewInstance | PropAccess | ObjectLiteral | ArrayLiteral | ImportDecl | CurriedFunction | PartialApplication | LazyExpr | MemoizedFunction | MaybeType | EitherType;
...
export interface LambdaValue { __tag: 'lambda'; params: string[]; body: Expression; closure: Env; __native?: (...args: any[]) => any; }
