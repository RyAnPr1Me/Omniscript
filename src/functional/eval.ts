...
function evalExpr(expr: Expression, env: Env): any {
  switch (expr.type) {
    ...
    case "Identifier":
      return envLookup(env, (expr as Identifier).name);
    case "Prop": {
    ...
    case "Call": {
      ...
      for (let i = 0; i < calleeVal.params.length; i++)
        envDefine(callEnv, calleeVal.params[i], evaluatedArgs[i]);
    ...
    case "Partial": {
      ...
      return createPartialFunction(
        fn,
        pa.args.map((arg) => (arg ? evalExpr(arg, env) : undefined)),
      );
    }
    case "Lazy": {
    ...
    case "Pipe": {
      ...
      if (p.right.type === "Identifier") {
        ...
        envDefine(callEnv, fn.params[0], left);
      ...
      if (p.right.type === "Call") {
        ...
        for (let i = 0; i < call.args.length; i++)
          envDefine(callEnv, callee.params[i + 1], evalExpr(call.args[i], env));
    ...
    case "New": {
      ...
      Object.keys(cls.methods).forEach((k) => {
        ...
        if (typeof methodDef === "function") {
          ...
          const lv: LambdaValue = {
            __tag: "lambda",
            params: methodDef.params,
            body: methodDef.body,
            closure: createEnv(env),
          };
          ...
          for (const importName of i.imports) {
            if (stdlib[importName as keyof typeof stdlib]) {
              envDefine(
                env,
                importName,
                stdlib[importName as keyof typeof stdlib],
              );
            }
          }
        }
...
function installBuiltins(env: Env) {
  const make = (name: string, params: string[], impl: (...a: any[]) => any) => {
    const lv: LambdaValue = {
      __tag: "lambda",
      params,
      body: { type: "Identifier", name: "__native" } as any,
      closure: createEnv(),
    };
  ...
  const mockDatabase = {
    users: {
      ...
      where: (predicate: any) => ({
        orderBy: (field: any) => ({
          take: (count: number) =>
            Promise.resolve([{ id: 1, name: "John Doe", posts: [1, 2] }]),
        }),
      }),
    },
  };
  ...
  make("partial", ["fn", "args"], (fn: LambdaValue, args: any[]) =>
    createPartialFunction(fn, args),
  );
  make("memoize", ["fn"], (fn: LambdaValue) =>
    createMemoizedFunction(fn as any),
  );
  make("identity", ["x"], (x: any) => x);
  make("constant", ["x"], (x: any) => ({
    __tag: "lambda",
    params: ["_"],
    body: { type: "Identifier", name: "__native" } as any,
    closure: createEnv(),
  ...
  make("flip", ["fn"], (fn: LambdaValue) => ({
    __tag: "lambda",
    params: ["a", "b"],
    body: { type: "Identifier", name: "__native" } as any,
    closure: createEnv(),
    __native: (a: any, b: any) => invoke2(fn, b, a),
  }));
  ...
  make("sortBy", ["fn", "arr"], (fn: LambdaValue, arr: any[]) =>
    [...arr].sort((a, b) => invoke1(fn, a) - invoke1(fn, b)),
  );
  ...
  make("takeC", ["n"], (n: number) => ({
    __tag: "lambda",
    params: ["arr"],
    body: { type: "Identifier", name: "__native" } as any,
    closure: createEnv(),
    __native: (arr: any[]) => arr.slice(0, n),
  }));
  make("dropC", ["n"], (n: number) => ({
    __tag: "lambda",
    params: ["arr"],
    body: { type: "Identifier", name: "__native" } as any,
    closure: createEnv(),
    __native: (arr: any[]) => arr.slice(n),
  }));
  make("zip", ["arr1", "arr2"], (arr1: any[], arr2: any[]) =>
    arr1.map((x, i) => [x, arr2[i]]).filter(([_, y]) => y !== undefined),
  );

  // Lists / higher-order
  make("range", ["n"], (n: number) => Array.from({ length: n }, (_, i) => i));
  make("map", ["arr", "fn"], (arr: any[], fn: LambdaValue) =>
    arr.map((v) => invoke1(fn, v)),
  );
  make("filter", ["arr", "fn"], (arr: any[], fn: LambdaValue) =>
    arr.filter((v) => invoke1(fn, v)),
  );
  make(
    "reduce",
    ["arr", "init", "fn"],
    (arr: any[], init: any, fn: LambdaValue) =>
      arr.reduce((acc, v) => invoke2(fn, acc, v), init),
  );
  make("compose", ["f", "g"], (f: LambdaValue, g: LambdaValue) => ({
    __tag: "lambda",
    params: ["x"],
    body: { type: "Identifier", name: "__native" } as any,
    closure: createEnv(),
    __native: (x: any) => invoke1(f, invoke1(g, x)),
  }));
  make("pipe", ["g", "f"], (g: LambdaValue, f: LambdaValue) => ({
    __tag: "lambda",
    params: ["x"],
    body: { type: "Identifier", name: "__native" } as any,
    closure: createEnv(),
...
function createPartialFunction(
  fn: LambdaValue,
