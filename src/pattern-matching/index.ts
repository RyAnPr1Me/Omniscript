...
export class PatternMatcher {
  ...
  match(value: any, cases: MatchCase[]): any {
    ...
    throw new OmniscriptError(
      "Non-exhaustive pattern match - no case matched the value",
    );
  }
  ...
  analyzeExhaustiveness(cases: MatchCase[], valueType: string): MatchAnalysis {
    ...
    if (!analysis.isExhaustive) {
      analysis.warnings.push(
        "Pattern match is not exhaustive - some values may not be handled",
      );
    }
  ...
  private matchConstructor(
    ...
    if (pattern.patterns) {
      ...
      return pattern.patterns.every((subPattern, index) => {
        const fieldValue =
          (value as any)[`field${index}`] || (value as any)[index];
  ...
  private patternSubsumes(pattern1: Pattern, pattern2: Pattern): boolean {
    ...
    switch (pattern1.type) {
      ...
      case "array":
        return this.patternsSubsume(
          pattern1.patterns || [],
          pattern2.patterns || [],
        );
      case "object":
        return this.objectPatternSubsumes(
          pattern1.properties || {},
          pattern2.properties || {},
        );
      default:
  ...
  private objectPatternSubsumes(
    props1: Record<string, Pattern>,
    props2: Record<string, Pattern>,
  ): boolean {
  ...
  private findMissingPatterns(
    ...
    switch (valueType) {
      case "boolean": {
        ...
        const hasFalse = cases.some((c) =>
          this.matchesLiteral(c.pattern, false),
        );
      ...
      case "Option": {
        const hasSome = cases.some((c) =>
          this.matchesConstructor(c.pattern, "Some"),
        );
        const hasNone = cases.some((c) =>
          this.matchesConstructor(c.pattern, "None"),
        );
        const hasOptionWildcard = cases.some(
          (c) => c.pattern.type === "wildcard",
        );
...
export class MatchCompiler {
  compile(matchExpr: { value: any; cases: MatchCase[] }): any {
    ...
    if (analysis.redundantCases.length > 0) {
      console.warn(
        `Warning: Redundant cases found: ${analysis.redundantCases.join(", ")}`,
      );
    }
