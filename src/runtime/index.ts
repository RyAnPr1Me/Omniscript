...
export class Runtime {
  ...
  private executeClassDeclaration(node: {
    name: string;
    methods?: Array<{
      name: string;
      body?: { body?: Bytecode[]; statements?: Bytecode[] };
    ...
    for (const m of node.methods || []) {
      ...
      const fn = function (this: unknown, ...args: unknown[]) {
        ...
        try {
          ...
          (m.params || []).forEach((p, i: number) =>
            runtime.setVar(p.name, args[i]),
          );
          try {
      ...
      if (m.isOperator && m.operatorSymbol) {
        operatorMap[m.operatorSymbol] = function (
          this: unknown,
          other: unknown,
        ) {
  ...
  async executeAsync(bytecode: Bytecode): Promise<unknown> {
    try {
      switch (bytecode.type) {
        ...
        case "Return": {
          const arg = (bytecode as any).argument
            ? this.evalExpr((bytecode as any).argument)
            : undefined;
  ...
  private executeReturn(ret: Bytecode): unknown {
    ...
    const arg = (ret as any).argument
      ? this.evalExpr((ret as any).argument)
      : undefined;
    throw { __return: true, value: arg };
  }
  ...
  private async executeReturnAsync(ret: Bytecode): Promise<unknown> {
    return ret.value !== undefined
      ? await this.executeAsync(ret.value as any)
      : undefined;
  }
  ...
  private initializeStdlibGlobals(): void {
    try {
      ...
      this.scope.set("map", (arr: any[], fn: (...args: any[]) => any) =>
        arr.map(fn),
      );
      this.scope.set("filter", (arr: any[], fn: (...args: any[]) => any) =>
        arr.filter(fn),
      );
      this.scope.set(
        "reduce",
        (arr: any[], initial: any, fn?: (...args: any[]) => any) => {
      ...
      this.scope.set("forEach", (arr: any[], fn: (...args: any[]) => any) =>
        arr.forEach(fn),
      );
      this.scope.set("find", (arr: any[], fn: (...args: any[]) => any) =>
        arr.find(fn),
      );
      this.scope.set("findIndex", (arr: any[], fn: (...args: any[]) => any) =>
        arr.findIndex(fn),
      );
      this.scope.set("includes", (arr: any[], item: any) => arr.includes(item));
      this.scope.set("indexOf", (arr: any[], item: any) => arr.indexOf(item));
      this.scope.set("join", (arr: any[], separator?: string) =>
        arr.join(separator),
      );
      this.scope.set("slice", (arr: any[], start?: number, end?: number) =>
        arr.slice(start, end),
      );
      this.scope.set("flatMap", (arr: any[], fn: (...args: any[]) => any) =>
        arr.flatMap ? arr.flatMap(fn) : arr.map(fn).flat(),
      );
      this.scope.set("flat", (arr: any[], depth?: number) =>
        arr.flat ? arr.flat(depth) : arr,
      );
      ...
      this.scope.set("head", (arr: any[]) =>
        arr.length > 0 ? arr[0] : undefined,
      );
      this.scope.set("tail", (arr: any[]) =>
        arr.length > 1 ? arr.slice(1) : [],
      );
      this.scope.set("last", (arr: any[]) =>
        arr.length > 0 ? arr[arr.length - 1] : undefined,
      );
      this.scope.set("init", (arr: any[]) =>
        arr.length > 1 ? arr.slice(0, -1) : [],
      );
      this.scope.set("cons", (head: any, tail: any[]) => [head, ...tail]);
      this.scope.set(
        "flip",
        (fn: (a: any, b: any) => any) => (b: any, a: any) => fn(a, b),
      );
      ...
      this.scope.set("apply", (fn: (...args: any[]) => any, args: any[]) =>
        fn(...args),
      );
      ...
      this.scope.set(
        "sort",
        (arr: any[], compareFn?: (a: any, b: any) => number) => {
          if (!Array.isArray(arr)) {
      ...
      this.scope.set("split", (str: string, separator: string) =>
        str.split(separator),
      );
      this.scope.set(
        "replace",
      ...
      this.scope.set("startsWith", (str: string, prefix: string) =>
        str.startsWith(prefix),
      );
      this.scope.set("endsWith", (str: string, suffix: string) =>
        str.endsWith(suffix),
      );
      this.scope.set("charAt", (str: string, index: number) =>
        str.charAt(index),
      );
      this.scope.set("substring", (str: string, start: number, end?: number) =>
  ...
  allocate(object: object): void {
    ...
    this.referenceCounts.set(
      object,
      (this.referenceCounts.get(object) || 0) + 1,
    );
  ...
  public getMemoryUsage(): { allocated: number; references: number } {
    return {
      ...
      references: Array.from(this.referenceCounts.values()).reduce(
        (a, b) => a + b,
        0,
      ),
    };
  ...
  createActor<TState>(
    actorFn: (message: any, state: TState) => TState | Promise<TState>,
    initialState: TState,
  ): Actor<TState> {
  ...
  private executeTry(node: {
    tryBlock: Bytecode;
    catchBlock?: Bytecode;
    catchVar?: string;
    finallyBlock?: Bytecode;
  }): unknown {
    try {
      ...
      if (node.catchBlock) {
        ...
        if (node.catchVar) {
          ...
          const catchValue = (err as any).__omniscriptThrow
            ? (err as any).__value
            : err;
        ...
        if (Object.keys(bindings).length > 0) {
          ...
          try {
            return this.evalExpr(
              matchCase.action || matchCase.value || matchCase.expression,
            );
          } finally {
            this.popEnv();
          }
  ...
  private evalExpr(
    expr: Bytecode | { type?: string; kind?: string; [key: string]: unknown },
  ): unknown {
    ...
    switch (k) {
      ...
      case "Call": {
        ...
        if (callee.kind === "MemberAccess") {
          ...
          if (fn && typeof fn === "object" && (fn as any).__tag === "lambda") {
            ...
            const callEnv = this.createLambdaEnv(
              lambda.closure || new Map(),
              lambda.params,
              args,
            );
      ...
      case "Ternary":
        return this.evalExpr((expr as any).condition)
          ? this.evalExpr((expr as any).trueExpr)
          : this.evalExpr((expr as any).falseExpr);
      case "ArrayLiteral":
  ...
  private evalMatch(expr: {
    subject: any;
    matchArms?: Array<{ pattern: any; guard?: any; value: any }>;
  }): unknown {
  ...
  private evalTemplateLiteral(expr: {
    parts?: (any | string)[];
    expressions?: any[];
  }): string {
  ...
  private matchPattern(
    value: unknown,
    pattern: { kind: string; value?: unknown },
  ): boolean {
    ...
    if ((pattern as any).kind === "literal")
      return value === (pattern as any).value;
    return false;
  }

  private executeImport(node: {
    source?: string;
    from?: string;
      ...
      if (importSpecs && importSpecs.length > 0) {
        ...
        for (const spec of importSpecs) {
          if (stdlib[spec.imported as keyof typeof stdlib]) {
            ...
            this.setVar(
              spec.local,
              stdlib[spec.imported as keyof typeof stdlib],
            );
          } else {
            logger.warn(
              "Runtime",
              `Module '${spec.imported}' not found in stdlib`,
            );
          }
        }
        return imported;
    ...
    logger.warn(
      "Runtime",
      `Module imports from '${node.source}' not fully implemented`,
    );
    return {};
  }
  ...
  private loadOmniscriptStdlib(): any {
    try {
      ...
      if (!fs.existsSync(omnistdlibPath)) {
        logger.debug(
          "Runtime",
          "Omniscript stdlib directory not found, falling back to TypeScript stdlib",
        );
        return null;
      }
      ...
      for (const moduleName of coreModules) {
        ...
        if (fs.existsSync(modulePath)) {
          try {
            ...
            const moduleExports = this.executeOmniscriptModule(
              moduleSource,
              moduleName,
            );
            ...
            logger.info(
              "Runtime",
              `Loaded Omniscript stdlib module: ${moduleName}`,
            );
          } catch (error) {
            logger.warn(
              "Runtime",
              `Failed to load Omniscript module ${moduleName}: ${error}`,
            );
          }
        }
      }
  ...
  private executeOmniscriptModule(source: string, moduleName: string): any {
    try {
      ...
      logger.debug(
        "Runtime",
        `Omniscript module ${moduleName} exported: ${Object.keys(exports).join(", ")}`,
      );
      return exports;
    } catch (error) {
  ...
  private evalBinary(expr: {
    ...
    if (
      ...
      typeof l === "object" &&
      (l as any).__ops &&
      typeof (l as any).__ops[expr.operator] === "function"
    ) {
    ...
    switch (expr.operator) {
      ...
      case "||":
        return (l as any) || (r as any);
      case "??=":
        return l ?? this.evalAssign((expr as any).left, r);
      default:
