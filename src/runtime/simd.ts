...
export class SIMDProcessor implements SIMDOperations {
  ...
  fma(a: number[], b: number[], c: number[]): number[] {
    ...
    debug.debug('SIMD', `Performing fused multiply-add on arrays of length ${a.length}`);
  ...
  reduce(a: number[], operation: 'sum' | 'min' | 'max' | 'mean'): number {
    ...
    debug.debug('SIMD', `Performing ${operation} reduction on array of length ${a.length}`);
  ...
  transform(a: number[], fn: (x: number) => number): number[] {
    debug.debug('SIMD', `Performing transformation on array of length ${a.length}`);
  ...
  convolve(signal: number[], kernel: number[]): number[] {
    debug.debug('SIMD', `Performing convolution: signal(${signal.length}) * kernel(${kernel.length})`);
  ...
  crossCorrelation(a: number[], b: number[]): number[] {
    debug.debug('SIMD', `Performing cross-correlation between arrays of length ${a.length} and ${b.length}`);
