...
export class TypeScriptDocGenerator {
  ...
  private initializeProgram(): void {
    ...
    debug.debug('DocGenerator', `Initialized with ${this.sourceFiles.length} source files`);
  }
  ...
  generateDocumentation(): APIModule[] {
    ...
    for (const sourceFile of this.sourceFiles) {
      ...
      if (module.classes.length > 0 || module.interfaces.length > 0 || module.functions.length > 0) {
        modules.push(module);
      }
    }

    debug.debug('DocGenerator', `Generated documentation for ${modules.length} modules`);
    return modules;
  }
  ...
  private processClass(node: ts.ClassDeclaration, sourceFile: ts.SourceFile): APIClass | null {
  ...
  private processInterface(node: ts.InterfaceDeclaration, sourceFile: ts.SourceFile): APIInterface | null {
  ...
  private processFunction(node: ts.FunctionDeclaration, sourceFile: ts.SourceFile): APIFunction | null {
    if (!node.name) return null;
  ...
  private processMethod(node: ts.MethodDeclaration, sourceFile: ts.SourceFile): APIMethod | null {
  ...
  private processProperty(node: ts.PropertyDeclaration, sourceFile: ts.SourceFile): APIProperty | null {
    if (!node.name || !ts.isIdentifier(node.name)) return null;
  ...
  private processMethodSignature(node: ts.MethodSignature, sourceFile: ts.SourceFile): APIMethod | null {
    if (!node.name || !ts.isIdentifier(node.name)) return null;
  ...
  private processPropertySignature(node: ts.PropertySignature, sourceFile: ts.SourceFile): APIProperty | null {
    if (!node.name || !ts.isIdentifier(node.name)) return null;
  ...
  private extractDocumentation(node: ts.Node, sourceFile: ts.SourceFile): DocComment {
  ...
  private getJSDocComments(node: ts.Node, sourceFile: ts.SourceFile): string | null {
  ...
  private extractParams(jsDoc: string): Array<{ name: string; type: string; description?: string }> {
  ...
  private extractReturns(jsDoc: string): { type: string; description?: string } | undefined {
  ...
  private getModuleName(sourceFile: ts.SourceFile): string {
    ...
    return fileName === 'index' ? path.basename(path.dirname(sourceFile.fileName)) : fileName;
  }
  ...
  private hasModifier(node: ts.Node, kind: ts.SyntaxKind): boolean {
    ...
    return nodeWithModifiers.modifiers?.some((modifier: any) => modifier.kind === kind) ?? false;
  }
  ...
  private getParameters(parameters: ts.NodeArray<ts.ParameterDeclaration>): Array<{ name: string; type: string; optional: boolean }> {
